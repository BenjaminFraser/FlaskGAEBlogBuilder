...
def UserPosts(user_id):
    """Allow the user to view all created, followed and liked posts.
    Args:
        user_id: the unique id that relates to the current user.
    Returns:
        A redirect to the User's post page, or /login, dependent
        on login state of the user. If the user is not logged in, return login page. 
    """
    if request.method == 'GET':
        # validate users id and current login state
        # query user's liked keys, disliked keys, liked and disliked posts and followers.
        # return a flask template with the required objects from Google Datastore.
